...
<script>
	export default {
		...
		methods: {
			//扁平化树结构
			renderTreeList(list=[], rank=0, PKeyID=[]){
				let that = this
				list.forEach(item=>{
					that.treeList.push({
						KeyID: item.KeyID,
			...
			treeItemTap(item){
				...
				list.forEach(childItem=>{
					if(item.showChild === false){
						//隐藏所有子级
						if(!childItem.PKeyID.includes(KeyID)){
							return;
						}
						if(childItem.lastRank !== true){
						...
						if(childItem.PKeyID[childItem.PKeyID.length-1] === KeyID){
							childItem.show = true;
						}
					}
				})
