...
<script>
	export default {
		...
		methods: {
			//扁平化树结构
			renderTreeList(list=[], rank=0, Par_GUID=[]){
				let that = this
				list.forEach(item=>{
					that.treeList.push({
						GUID: item.GUID,
			...
			treeItemTap(item){
				...
				list.forEach(childItem=>{
					if(item.showChild === false){
						//隐藏所有子级
						if(!childItem.Par_GUID.includes(GUID)){
							return;
						}
						if(childItem.lastRank !== true){
						...
						if(childItem.Par_GUID[childItem.Par_GUID.length-1] === GUID){
							childItem.show = true;
						}
					}
				})
